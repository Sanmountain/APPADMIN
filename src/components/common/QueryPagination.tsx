import { IQueryPaginationProps } from "../../types/QueryPagination.types";
...
export default function QueryPagination({
  ...
  const onClickPrevButton = () => {
    if (page === currentPageGroup * pagesPerGroup + 1 && currentPageGroup > 0) {
      ...
      setPage((currentPageGroup - 1) * pagesPerGroup + pagesPerGroup);
  ...
  const onClickNextButton = () => {
    ...
    if (isAtEndOfCurrentGroup && isNotLastGroup) {
      ...
      const firstPageOfNextGroup = (currentPageGroup + 1) * pagesPerGroup + 1;
      setPage(firstPageOfNextGroup);
      setCurrentPageGroup(currentPageGroup + 1);
    } else {
      // NOTE 다음 페이지로 이동
      setPage(page + 1);
